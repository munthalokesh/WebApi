...
namespace WebApi.Models.DataLayer
{
    public class PlanesDbOperations
    {
        public static AddressToClient GetAddress(string email)
        {
            ...
            var address = entities.Owner.FirstOrDefault(x => x.Email == email);
            if (address == null)
            {
                return null;
            }
        ...
        {
            AirportManagementEntities AE = new AirportManagementEntities();
            var id = AE.Owner.OrderByDescending(item => item.OwnerId).FirstOrDefault();
            if (id == null)
            {
                return 104;
            }
        ...
        public string AddPlane(PlaneDetails p)
        {
            try
            {
                using (var context = new AirportManagementEntities())
                {
                    var owner = context.Owner.FirstOrDefault(x => x.Email == p.Email);
